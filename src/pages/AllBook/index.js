          ...
          {slice?.map((el) => {
            return (
              <Card key={el.id} style={{ marginTop: "1vh" }}>
                ...
                <Card.Body>
                  <Card.Title className="allBookCardTitle">
                    {el.data.title}
                  </Card.Title>
                  <Card.Subtitle className="allBookCardSubTitle">
                    {el.data.author}
                  </Card.Subtitle>
                  <Card.Text className="allBookPrice">
                    {Number(el.data.price).toLocaleString("en-US")} VND
                  </Card.Text>
                  <Card.Text style={{ display: "flex" }}>
                    Genre:
                    <Container className="allBookTagContainer">
                      {el.data.genre[0]}
                    </Container>
                    <Container className="allBookTagContainerEtc">
                      ...
                    </Container>
                  </Card.Text>
                  <Button
