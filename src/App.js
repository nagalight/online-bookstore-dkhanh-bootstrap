...
function App() {
  ...
  const [showNavBarWebView, setShowNavBarWebView] = useState("none");
  const [showNavBarResponsive, setShowNavBarResponsive] = useState("none");

  useEffect(() => {
    if (windowSize[0] > 768) {
      setShowNavBarWebView("block");
      setShowNavBarResponsive("none");
    } else if (windowSize[0] <= 768) {
      setShowNavBarWebView("none");
      setShowNavBarResponsive("block");
    }
  }, [windowSize]);
  ...
  return !isLoading ? (
    <>
      <BrowserRouter>
        ...
        <Routes>
          ...
          <Route
            path="/genres/:genre"
            element={<GenrePage handleAddToCart={handleAddToCart} />}
          />
          <Route
            exact
            path="/books"
            element={<AllBook handleAddToCart={handleAddToCart} />}
          />
          <Route
            exact
            path="/books/:id"
          ...
          <Route
            ...
            element={<SearchResult handleAddToCart={handleAddToCart} />}
          />
          <Route
            path="/payment"
            element={
    ...
    <div id="spinner" className="loadingWrapper">
      <img
        ...
        src="https://firebasestorage.googleapis.com/v0/b/za-library-account.appspot.com/o/Main%2FZA_icon.png?alt=media&token=d281410f-e149-46b4-bf1a-b3e9375f62fd"
        alt="Za_Icon"
      />
