...
function GenreSlider3() {
  ...
  const fetchBookData = () => {
    const q = query(
      collection(db, "books"),
      where("genre", "array-contains", "Magazine"),
      limit(7)
    );
  ...
  return (
    <Container className="homeBookSliderWrapper">
      <Container className="homeBookSliderTitleWrapper">
        <Container className="homeBookSliderTitleContainer">
          Get Up-to-date with Magazine:
        </Container>
          ...
          {genreBookData?.map(({ id, data }) => {
            return (
              <Card key={id}>
                ...
                <Card.Body>
                  <Card.Title className="homeBookCardBookTitle">
                    {data.title}
                  </Card.Title>
                  <Card.Subtitle className="homeBookCardBookSubtitle">
                    {data.author}
                  </Card.Subtitle>
                  <Card.Text className="homeBookPrice">
                    {Number(data.price).toLocaleString("en-US")} VND
                  </Card.Text>
                  <Card.Text className="homeBookCardGenre">
                    Genre:
                    <Container className="homeTagContainer">
                      {data.genre[0]}
                    </Container>
